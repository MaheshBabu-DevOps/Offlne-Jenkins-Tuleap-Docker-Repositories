#Automated Jenkins Plugin Downloader for Latest Versions
...
# Define plugins you want to download
declare -a plugins=(
...
download_plugin() {
    plugin_name=$1
    echo "Downloading $plugin_name version latest..."
    curl -L -o "$download_dir/$plugin_name.hpi" "https://updates.jenkins-ci.org/download/plugins/$plugin_name/latest/$plugin_name.hpi"
    if [ $? -eq 0 ]; then
...
# Loop through the plugins and download each one
for plugin in "${plugins[@]}"; do
    download_plugin "$plugin"
done

echo "All requested plugins downloaded to $download_dir."

# Create the ZIP file
cd "$download_dir"
zip -r jenkins_plugins_backup.zip *
