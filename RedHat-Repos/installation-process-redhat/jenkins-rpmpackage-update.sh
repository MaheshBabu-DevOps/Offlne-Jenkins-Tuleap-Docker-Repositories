...
sudo cp -r sudo cp -r /run/media/cair/USB/Downloads/jenkins-2.479.1-1.1.noarch.rpm repodata/   /media/cair/jenkins_repo

sudo tee /etc/yum.repos.d/local-repo.repo > /dev/null <<EOL
# Jenkins Local Repository
[jenkins-local]
name=Jenkins Local Repository
baseurl=file:///media/cair/jenkins_repo/
gpgcheck=0
enabled=1
EOL
